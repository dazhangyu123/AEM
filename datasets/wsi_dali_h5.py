...
def get_split_from_df(slide_data, all_splits, split_key='train'):
    ...
    if len(split) > 0:
        ...
        return None
label_dict={0: 0, 1: 1,2:2,3:3,4:3,5:3, 'IDC': 0, 'ILC': 1}
# label_dict={'IDC': 0, 'ILC': 1,2:1,3:1,4:1,5:1}
def get_data_list(data_dir, split_num, train_val_test, splitter_path, label_csv_path):
    ...
    for i, slide in enumerate(slide_id_list):
        wsi_path = os.path.join(data_dir, slide + '.h5')
...
class ExternalInputIterator(object):
    def __init__(self, data_dir, batch_size, split_num, splitter_path, shuffle=False, device_id=0, num_gpus=1, train_eval_test='val',
        ...
        # self.cur_split =
        self.wsi_list_all, self.label_list_all, self.roi_rankings_all = get_data_list(data_dir, split_num,
                                                                                      self.train_val_test,
                                                                                      splitter_path,
        ...
        for _ in range(self.batch_size):
            ...
            for _ in range(self.batch_size):
                ...
                with h5py.File(full_path, 'r') as hdf5_file:
                    instance_num = len(hdf5_file['patches'])
                    if instance_num <= self.bag_size:
                        topk_indices = range(self.bag_size)
                    else:
                        try:
                            rois_attention_scores = torch.tensor(np.load(self.roi_rankings[sample_idx]))
                        except:
                            pdb.set_trace()
                        # print(rois_attention_scores.shape)
                        topk_indices = rois_attention_scores.topk(self.bag_size)[1].squeeze()

                    for j in range(self.bag_size):
                        if j < instance_num:
                            batch.append(hdf5_file['patches'][topk_indices[j]])
                ...
                # pdb.set_trace()
                # labels += [np.array([self.label_list[sample_idx], 1], dtype=np.uint8)]*self.batch_size
            return (batch, labels,)
...
def get_train_val_test_loaders(data_dir,splitter_path, bag_size=256, label_csv_path='./',split_num=0):
...
if __name__ == '__main__':
    ...
    #     # images,label = data
    #     bag_img_each = data[0]["data"]
    #     # wsi_label_each = data[0]["label"][::1024].long().cuda()
    #     print('idx', i, bag_img_each.shape)