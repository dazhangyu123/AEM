...
def split_dataset_huaxi(file_path, conf):
    # csv_path = './dataset_csv/huaxi.csv'
    # df = pd.read_csv(csv_path)
    # df = df.dropna(subset=['therapy'])
    # condition = df['therapy'].str.contains('无新')
    # # Use the drop method to delete rows based on the condition
    # df = df[condition].reset_index(drop=True)

    h5_data = h5py.File(file_path, 'r')
    slide_names = list(h5_data.keys())
    # slide_names = [name for name in slide_names if '2023-10' not in name]
    # slide_names = [name for name in slide_names if name in df['slide_id'].values]


    train_val_names, test_names = train_test_split(slide_names, test_size=0.2)
    train_names, val_names = train_test_split(train_val_names, test_size=0.01)
