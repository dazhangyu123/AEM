...
def get_arguments():
    ...
    parser.add_argument('--config', dest='config', default='config/brca_er_config.yml',
    ...
    parser.add_argument('--pretrain', default='conch', choices=['natural_supervised', 'medical_ssl', 'plip', 'path-clip-B-AAAI', 'titan',
...
def main():
    ...
    if conf.pretrain == 'medical_ssl':
        ...
        conf.D_feat = 512
        conf.D_inner = 256
    elif conf.pretrain == 'path-clip-L-336' or conf.pretrain == 'openai-clip-L-336' or conf.pretrain == 'titan':
        conf.D_feat = 768
        conf.D_inner = 384
    ...
    wandb.init(
        ...
        config={'dataset': conf.config.split('/')[-1].replace("_config.yml", ""),
    ...
    train_loader = DataLoader(train_data, batch_size=conf.B, shuffle=True,
                              num_workers=conf.n_worker, pin_memory=conf.pin_memory, drop_last=True, persistent_workers=False)
    val_loader = DataLoader(val_data, batch_size=conf.B, shuffle=False,
                             num_workers=conf.n_worker, pin_memory=conf.pin_memory, drop_last=False, persistent_workers=False)
